...
(function (factory) {
    ...
    FileInput.prototype = {
        ...
        _init: function (options) {
            ...
            self.reader = null;
            self.formdata = {};
            self.cubeGroupFiles = {};//liyfa çƒé¢å…¨æ™¯
            self.clearStack();
            self.uploadCount = 0;
            self.uploadStatus = {};
            self.uploadLog = [];
        ...
        //20160720 liyfa æŒ‡å®šè‡ªå®šä¹‰çš„ä¸Šä¼ æŒ‰é’®ï¼Œè®©æ’ä»¶æŽ§åˆ¶å¯ç”¨æˆ–ä¸å¯ç”¨çš„çŠ¶æ€
        _ivrSpecify$btnUpload: function (selector) {
            var self = this;
            self.$btnUpload = $(selector);
        },
        _appendCubeFiles: function (cubeGroupId,groupObj) {
            var self = this;
            self.cubeGroupFiles[cubeGroupId] = groupObj;
        },
        _removeCubeFiles: function (cubeGroupId) {
            var self = this;
            delete self.cubeGroupFiles[cubeGroupId];
        },
        _getCubeFiles: function () {
            var self = this;
            return self.cubeGroupFiles;
        },
        _validate: function () {
        ...
        _uploadSingle: function (i, files, allFiles) {
            ...
            self._raise('filepreajax', [previewId, i]);
            self._uploadExtra(previewId, i);
        ...
        _uploadBatch: function () {
            var self = this, files = self.filestack,cubes = self.cubeGroupFiles, total = files.length, params = {}, fnBefore, fnSuccess, fnError,
            ...
            if(Object.getOwnPropertyNames(cubes).length > 0){
                $.each(cubes,function(cubeid,groupObj){
                    $.each(groupObj,function(key,file){
                        self.formdata.append(cubeid, file, key+file.name.substring(file.name.lastIndexOf('.')));
                    });
                });
            }
        ...
        _uploadExtraOnly: function () {
            var self = this,cubes = self.cubeGroupFiles, params = {}, fnBefore, fnSuccess, fnComplete, fnError;
            self.formdata = new FormData();
            if (self._abort(params)) {
                return;
            }
    ...
    $.fn.fileinput.defaults = {
        ...
        previewZoomButtonIcons: {
            prev: '<i class="icon icon-long-arrow-left"></i>',
            next: '<i class="icon icon-long-arrow-right"></i>',
            toggleheader: '<i class="icon icon-resize"></i>',
